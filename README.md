...
- Kanban — panneau de tâche complet (30/08/2026, phase 1 de l'amélioration du Kanban) : cliquer sur une carte ouvre un panneau détaillé (titre, description, échéance avec badge "En retard", assigné et priorité modifiables, commentaires). Un commentaire peut taguer une ou plusieurs personnes (chips, pas de parsing "@Nom") — la personne taguée reçoit une notification (in-app + e-mail, `mentions` sur `commentaires`, 0017_kanban_mentions.sql) avec lien direct vers la tâche. Assignation et échéance dès la création rapide d'une tâche. Gestion des colonnes réservée à l'admin (renommer, réordonner, ajouter, supprimer — bloqué si la colonne contient encore des tâches). Restent pour une phase future, à la demande explicite : pièces jointes (nécessite Supabase Storage, pas encore en place) et "espaces de travail" Kanban (plusieurs tableaux distincts avec partage, façon Trello/Asana — pour l'instant un seul tableau par entreprise). La mention se saisit directement dans le champ (01/09/2026) : taper "@" fait apparaître une liste de suggestions filtrée au fil de la frappe, façon Slack/GitHub — sélection au clic ou au clavier (flèches, Entrée/Tab), pas de boutons de tag séparés.
- Kanban — espace personnel / espace partagé (01/09/2026) : jusqu'ici un collaborateur ne voyait sur le Kanban que ses propres tâches (assigné ou créateur), ce qui donnait l'impression d'un tableau personnel plutôt que d'équipe. Le tableau a maintenant deux onglets, mêmes colonnes pour les deux (`0018_kanban_espace.sql`, colonne `espace` sur `taches`) : "Espace partagé" (onglet par défaut) est visible et modifiable par tout le monde dans l'entreprise, façon tableau Trello d'équipe ; "Mon espace personnel" garde le comportement historique (visible/modifiable par le propriétaire, son manager ou un admin). Le lien de notification (mention en commentaire, assignation) ouvre automatiquement le bon onglet.
- Kanban — pièces jointes (02/09/2026, dernier chantier ajourné du Kanban) : bucket de stockage privé `taches-pieces-jointes` (15 Mo max par fichier, `0019_kanban_pieces_jointes.sql`) + table `pieces_jointes`, sécurisés par les mêmes règles de visibilité que la tâche elle-même (espace personnel/partagé). Depuis le panneau de détail d'une tâche : ajout (plusieurs fichiers à la fois), téléchargement (URL signée, le bucket n'est pas public), suppression immédiate — pas de corbeille pour les fichiers, contrairement au reste de l'app.
- Kanban — aperçu des pièces jointes (02/09/2026, retour d'Angelo après livraison des pièces jointes) : les images ont une vraie miniature dans la liste (pas juste un nom de fichier), et cliquer sur une pièce jointe ouvre un aperçu en fenêtre modale (image ou PDF affiché directement dans l'appli, pas de nouvel onglet) — les autres types de fichiers proposent le téléchargement faute d'aperçu possible. La première image jointe à une tâche sert aussi de miniature de couverture sur sa carte, façon Trello.
